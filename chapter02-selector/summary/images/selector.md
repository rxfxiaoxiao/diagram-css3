...
## 二、层次选择器(兼容程度良好，后面三个需要在IE7及以上版本支持)
...
### 4、UI元素状态伪类选择器，IE9及以上才支持

+ E:checked 表单元素选中状态
+ E:enabled 表单元素启用状态
+ E:disabled 表单元素禁用状态

### 5、结构伪类选择器（除了第一个，其他都是css3属性，所有的都是IE9及以上支持）
...
+ E:not(F) 匹配除了F以外的E元素。起到内容过滤作用

## 四、伪元素

+ ::first-letter 选择文本块的第一个字母，适用于首字母下沉
+ ::first-line 匹配元素的第一行文本
+ ::before 
+ ::after
+ ::selection 突出显示被用户选中的文本。
    - 只接受两个属性：background以及color
    - 兼容写法：E::-moz-selection: color:xx;background:xx; E::selection: color:xx;background:xx;

## 五、属性选择器

+ E[attr=val]  E元素定义了属性attr，且attr的属性值为val（``val区分大小写``）
+ E[attr|=val] E元素定义了属性attr，且attr的属性值为``val``或者以``val-``开头的属性值
+ E[attr~=val] E元素定义了属性attr，attr属性值具有``多个空格分隔``的值，其中一个值等于``val``
+ E[attr*=val] E元素定义了属性attr, attr属性值``任意位置包含了val``
+ E[attr^=val] E元素定义了属性attr, attr属性值是以``val开头``的任何字符串
+ E[attr$=val] E元素定义了属性attr, attr属性值是以``val结尾``的任何字符串
